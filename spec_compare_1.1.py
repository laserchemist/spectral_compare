...
# Define functions
def printMatrixE(a): # Clean screen print of matrix contents
   ...
   for i in range(0,rows):
      ...
      print()
   print()      
def vmult(a,b): # Vector multiplication for visual analysis of spectral cross-correlation
    rows=a.shape[0]
    cols=b.shape[0]
    prod=np.zeros((rows,cols))
    for i in range(0,rows):
      for j in range(0,cols):
          prod[i,j]=a[i]*b[j]
    return prod
...
while num<5:
    ...
    num+=1
# Do overall analysis
xs = np.linspace(500, 3600, 2100) # Define basis for spline grid
for i in range(num):
            for j in range(num):
                spli=UnivariateSpline(specx[i], specy[i],k=4,s=0)
                inti=spli.integral(xs.min(), xs.max())
                si=spli(xs)/inti
                splj=UnivariateSpline(specx[j], specy[j],k=4,s=0)
                intj=splj.integral(xs.min(), xs.max())
                sj=splj(xs)/intj
                #print("inti/j: ",inti,min(si),intj,min(sj))
                correl = np.corrcoef(si, sj) # Compared using same x = "xs"
                corr[i][j]=correl[1][0]
                c2=vmult(si,sj)
                cor=correlate(spli(xs),splj(xs))
                #print("shape of cross correlation: ",cor.shape,c2.shape)
                plt.imshow(c2, label=(onlycsv[option[i]]+":"+onlycsv[option[j]]),interpolation='bilinear', cmap=cm.nipy_spectral,
                origin='lower')
                #vmax=abs(c2).max(), vmin=abs(c2).min())
                #plt.legend(loc='best')
                #plt.colorbar()
                plt.savefig(filename+"_"+str(i)+str(j)+"_correlate.pdf",dpi=300)
                plt.close
...
if num>1:
    ...
    exit(print("No files specified, ending..."))
print("Peak data\n++++++++")
